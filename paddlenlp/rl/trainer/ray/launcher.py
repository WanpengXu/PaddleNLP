...
from paddlenlp.rl.trainer.ray.utils import ray_noset_visible_devices
...
class BaseDistributedRayActor:
    def __init__(self, world_size, rank, master_addr, master_port):
        ...
        self._master_endpoint = f"{self._master_addr}:{self._master_port}"
        ...
        # paddle env
        os.environ.update({
            "PADDLE_TRAINER_ID": str(self._rank),
            "PADDLE_TRAINERS_NUM": str(self._world_size),
            "PADDLE_CURRENT_ENDPOINT": self._master_endpoint,
            # "PADDLE_TRAINER_ENDPOINTS": ,     # 这个要等全部初始化后由WorkerGroup广播
            # "FLAGS_selected_gpus": "0",
            "FLAGS_selected_gpus": os.environ["LOCAL_RANK"], # 使用 Ray 分配的局部 GPU ID
        })
    ...
    def get_paddle_current_endpoint(self) -> str:
        """返回当前 RayActor 的 Paddle 分布式通信端点"""
        return self._master_endpoint

    def set_paddle_trainer_endpoints(self, trainer_endpoints: str):
        """由 RayActorGroup 远程调用，设置 PADDLE_TRAINER_ENDPOINTS 环境变量"""
        os.environ["PADDLE_TRAINER_ENDPOINTS"] = trainer_endpoints
        logging.info(f"Actor {self._rank} set PADDLE_TRAINER_ENDPOINTS={trainer_endpoints}")
    

class BaseModelRayActor(BaseDistributedRayActor):
    def _setup_distributed(self, strategy_config: dict):
        # configure strategy
        self.strategy = fleet.DistributedStrategy()
        # 配置 Paddle Fleet 分布式策略

        # 假定每个 Ray Actor 内部的 fleet 都是一个小的单节点分布式组
        self.strategy.hybrid_configs = {
            "dp_degree": strategy_config.get("dp_degree", 1),
            "mp_degree": strategy_config.get("mp_degree", 1),
            "sharding_degree": strategy_config.get("sharding_degree", 1),
            "pp_degree": strategy_config.get("pp_degree", 1),
        }

        # 先不实现 PP
        # self.strategy.pipeline_configs = {
        #     "accumulate_steps": strategy_config.get("gradient_accumulation_steps", 1),
        #     "micro_batch_size": strategy_config.get("per_device_train_batch_size", 1)
        # }
        
        fleet.init(is_collective=True, strategy=self.strategy)
        # 获取当前进程在当前 fleet group 中的 rank
        self.fleet_rank = fleet.worker_index()
        self.fleet_world_size = fleet.worker_num()
        logger.info(f"Paddle Fleet initialized: rank={self.fleet_rank}, world_size={self.fleet_world_size}")

    # 并行构建，根据 TP 和 PP 去改模型结构
    ...
    def execute_batch(self, method_name: str, all_data, start_idx, end_idx):
        ...
        results = []
        for i in tqdm(range(list_length), desc=f"{method_name}", disable=not (dist.get_rank() == 0)):
...
class RayActorGroup:
    ...
    def __init__(
        ...
        num_nodes,
        num_gpus_per_node,
        ray_actor_type: Type[BaseModelRayActor],
        ...
        # 在所有 Actor 实例创建并初始化后，协调 Paddle 环境变量
        self._setup_paddle_endpoints()
    ...
    def _setup_paddle_endpoints(self):
        """
        收集所有 Actor 的 Paddle Endpoint，并将其广播给所有 Actor。
        """
        logging.info("Collecting Paddle endpoints from all actors...")
        # 异步获取所有 Actor 的 PADDLE_CURRENT_ENDPOINT
        endpoint_refs = [actor.get_paddle_current_endpoint.remote() for actor in self._actor_handlers]
        all_endpoints: List[str] = ray.get(endpoint_refs)
        
        # 排序以确保所有 Actor 看到的 endpoints 列表顺序一致
        # 这对于分布式训练的稳定性非常重要
        all_endpoints.sort()

        # 构建 PADDLE_TRAINER_ENDPOINTS 字符串
        trainer_endpoints_str = ",".join(all_endpoints)
        logging.info(f"Collected all Paddle endpoints: {trainer_endpoints_str}")

        # 异步设置所有 Actor 的 PADDLE_TRAINER_ENDPOINTS
        set_endpoint_refs = [actor.set_paddle_trainer_endpoints.remote(trainer_endpoints_str)
                             for actor in self._actor_handlers]
        ray.get(set_endpoint_refs) # 等待所有 Actor 完成设置
        logging.info("Successfully set PADDLE_TRAINER_ENDPOINTS for all actors.")
